<x-secretaria-escolar-layout>
    <div class="space-y-8">
        <section class="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-3 text-3xl font-outfit font-bold text-slate-900">Horarios pedagogicos</h1>
                    <p class="mt-2 text-sm text-slate-600">Consulte, cadastre, edite e reorganize os horarios das turmas no contexto pedagogico da escola.</p>
                </div>
                @can('cadastrar horarios pedagogicamente')
                    <a href="{{ route('secretaria-escolar.coordenacao.horarios.create') }}" class="inline-flex items-center rounded-2xl bg-emerald-600 px-5 py-3 text-xs font-bold uppercase tracking-[0.18em] text-white transition hover:bg-emerald-700">
                        Nova grade pedagogica
                    </a>
                @endcan
            </div>
        </section>
        ...
        <section class="rounded-[2rem] border border-emerald-100 bg-white shadow-sm">
                    ...
                    <tbody>
                        ...
                        @forelse ($horarios as $horario)
                            <tr class="border-b border-slate-100">
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-slate-900">{{ $horario->turma->nome }}</p>
                                    <p class="mt-1 text-xs text-slate-500">{{ $horario->escola->nome }}</p>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    @canany(['editar horarios pedagogicamente', 'reorganizar horarios pedagogicamente'])
                                        <a href="{{ route('secretaria-escolar.coordenacao.horarios.edit', $horario) }}" class="inline-flex items-center rounded-2xl border border-emerald-200 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-emerald-700 transition hover:bg-emerald-50">
                                            Editar / reorganizar
                                        </a>
                                    @endcanany
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-slate-500">Nenhum horario encontrado para os filtros informados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
