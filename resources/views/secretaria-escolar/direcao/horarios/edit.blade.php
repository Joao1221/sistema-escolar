<x-secretaria-escolar-layout>
    <div class="space-y-8">
        <section class="rounded-[2rem] border border-indigo-100 bg-white p-6 shadow-sm">
            ...
            <p class="mt-2 text-sm text-slate-600">A direcao pode redistribuir horarios, docentes e ordem das aulas conforme necessidade da escola.</p>
        </section>

        <div class="rounded-[2rem] border border-indigo-100 bg-white p-6 shadow-sm">
            <form action="{{ route('secretaria-escolar.direcao.horarios.update', $horario) }}" method="POST" class="grid gap-6 md:grid-cols-2">
                @csrf
                @method('PUT')
                <div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div><x-input-label for="horario_inicial" :value="__('Horario inicial')" /><input id="horario_inicial" name="horario_inicial" type="time" value="{{ old('horario_inicial', \Carbon\Carbon::parse($horario->horario_inicial)->format('H:i')) }}" class="mt-1 block w-full rounded-xl border-slate-300" required></div>
                    <div><x-input-label for="horario_final" :value="__('Horario final')" /><input id="horario_final" name="horario_final" type="time" value="{{ old('horario_final', \Carbon\Carbon::parse($horario->horario_final)->format('H:i')) }}" class="mt-1 block w-full rounded-xl border-slate-300" required></div>
                </div>
                <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input id="ativo" name="ativo" type="checkbox" value="1" class="rounded border-slate-300 text-indigo-600" @checked(old('ativo', $horario->ativo))>
                ...
                <div class="md:col-span-2 flex justify-end gap-3">
                    ...
                    <button type="submit" class="inline-flex items-center rounded-2xl bg-indigo-600 px-5 py-3 text-xs font-bold uppercase tracking-[0.18em] text-white transition hover:bg-indigo-700">Salvar ajustes</button>
                </div>
            </form>
        </div>
    </div>
