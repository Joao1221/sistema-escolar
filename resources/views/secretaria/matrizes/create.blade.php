<x-secretaria-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Criar Matriz Curricular') }}
        </h2>
    </x-slot>

    <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-gray-100 p-8">
        <form action="{{ route('secretaria.matrizes.store') }}" method="POST" x-data="matrizForm()">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="md:col-span-2">
                    <x-input-label for="nome" :value="__('Nome da Matriz')" />
                    <x-text-input id="nome" name="nome" type="text" class="mt-1 block w-full" required autofocus placeholder="Ex: Matriz Ensino Fundamental - 1º ao 5º ano" />
                    <x-input-error class="mt-2" :messages="$errors->get('nome')" />
                </div>

                <div>
                    <x-input-label for="ano_vigencia" :value="__('Ano de Vigência')" />
                    <x-text-input id="ano_vigencia" name="ano_vigencia" type="number" class="mt-1 block w-full" required value="{{ date('Y') }}" />
                ...
                <div>
                    ...
                    <x-text-input id="serie_etapa" name="serie_etapa" type="text" class="mt-1 block w-full" required placeholder="Ex: 1º Ano" />
                </div>
            ...
            <div class="border-t border-gray-100 pt-6 mt-6">
                        ...
                        <template x-for="(item, index) in disciplinasSelecionadas" :key="index">
                            <tr class="border-t border-gray-100">
                                <td class="py-1.5 pr-2">
                                    <select :name="`disciplinas[${index}][id]`" class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded text-sm py-1.5" required>
                                        <option value="">Selecione...</option>
                                ...
                                <td class="py-1.5 px-1">
                                    <input type="number" :name="`disciplinas[${index}][carga_horaria]`" class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded text-sm py-1.5 text-center" required placeholder="h">
                                </td>
                                <td class="py-1.5 text-center">
                                    <input type="checkbox" :name="`disciplinas[${index}][obrigatoria]`" value="1" checked class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                </td>
                                <td class="py-1.5 text-center">
                ...
                <div x-show="disciplinasSelecionadas.length === 0" class="text-center py-6 bg-gray-50 rounded-xl border border-dashed border-gray-300 text-gray-400 text-sm">
                    Nenhuma disciplina vinculada à matriz ainda. Clique em "+ Vincular Disciplina" para começar.
                </div>
            </div>
