    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-gray-100 p-8 max-w-4xl">
        <form action="{{ isset($disciplina) ? route('secretaria.disciplinas.update', $disciplina) : route('secretaria.disciplinas.store') }}" method="POST">
            ...
            <div class="mt-8 flex items-center justify-end space-x-6">
                <a href="{{ route('secretaria.disciplinas.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 transition ease-in-out duration-150">Cancelar</a>
