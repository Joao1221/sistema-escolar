<x-secretaria-layout>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ $funcionario->nome }}</p>
        </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-gray-100">
        <div class="p-6">
            <form action="{{ route('secretaria.funcionarios.update', $funcionario) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="border-b border-gray-100 pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="nome" name="nome" type="text" class="mt-1 block w-full" :value="old('nome', $funcionario->nome)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nome')" />
                        </div>
                        <div>
                            <x-input-label for="cpf" :value="__('CPF')" />
                            <x-text-input id="cpf" name="cpf" type="text" class="mt-1 block w-full" :value="old('cpf', $funcionario->cpf)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('cpf')" />
                        </div>
                        <div>
                            <x-input-label for="email" :value="__('E-mail')" />
                            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $funcionario->email)" />
                            <x-input-error class="mt-2" :messages="$errors->get('email')" />
                        </div>
                        <div>
                            <x-input-label for="telefone" :value="__('Telefone')" />
                            <x-text-input id="telefone" name="telefone" type="text" class="mt-1 block w-full" :value="old('telefone', $funcionario->telefone)" />
                        ...
                        <div>
                            ...
                            <select id="cargo" name="cargo" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach (['Professor', 'Diretor', 'Coordenador', 'Secretário Escolar', 'Nutricionista', 'Psicólogo', 'Psicopedagogo', 'Auxiliar de Serviços Gerais', 'Merendeira', 'Vigilante'] as $cargo)
                                    <option value="{{ $cargo }}" {{ old('cargo', $funcionario->cargo) === $cargo || (\App\Support\CargosPsicossociais::contains(old('cargo', $funcionario->cargo)) && \App\Support\CargosPsicossociais::contains($cargo)) ? 'selected' : '' }}>{{ $cargo }}</option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <p id="escolas-help" class="text-sm text-gray-500 mb-4">Gerencie as escolas onde o funcionário atua.</p>
                    ...
                    <div id="escolas-wrapper" class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        @foreach ($escolas as $escola)
                            <label class="inline-flex items-center p-3 bg-white rounded border border-gray-200 cursor-pointer hover:bg-blue-50 transition">
                                <input type="checkbox" name="escolas[]" value="{{ $escola->id }}" class="escola-checkbox rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array($escola->id, old('escolas', $escolasSelecionadas)) ? 'checked' : '' }}>
                ...
                <div class="flex items-center justify-end border-t pt-6">
                    <x-primary-button>{{ __('Atualizar Funcionário') }}</x-primary-button>
                </div>
            </form>
        </div>
    </div>
