<x-nutricionista-layout :titulo="$tituloPagina" :subtitulo="$subtituloPagina" :breadcrumbs="$breadcrumbs">
    <div class="mx-auto max-w-5xl rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-sm">
        <form method="POST" action="{{ route('nutricionista.cardapios.update', $cardapio) }}" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="grid gap-5 md:grid-cols-2">
                <div>
                    <label class="text-xs font-semibold uppercase tracking-widest text-slate-500">Escola</label>
                    <select name="escola_id" class="mt-2 w-full rounded-xl border-slate-300 bg-slate-100 shadow-sm" disabled>
                        @foreach ($escolas as $escola)
                            <option value="{{ $escola->id }}" @selected($cardapio->escola_id == $escola->id)>{{ $escola->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="text-xs font-semibold uppercase tracking-widest text-slate-500">Data do cardapio</label>
                    <input type="date" name="data_cardapio" value="{{ old('data_cardapio', $cardapio->data_cardapio->format('Y-m-d')) }}" class="mt-2 w-full rounded-xl border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('nutricionista.cardapios.show', $cardapio) }}" class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancelar</a>
