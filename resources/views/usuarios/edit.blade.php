...
<x-secretaria-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-gray-100 p-6">
        <form method="POST" action="{{ route('secretaria.usuarios.update', $usuario->id) }}" class="space-y-5 max-w-2xl">
            ...
            @php
                $roleSelecionado = $perfis->firstWhere('id', old('role', $usuario->roles->first()?->id));
                $mostrarChefePsicossocial = $roleSelecionado?->name === $perfilPsicossocial;
            @endphp
            <div id="chefe-psicossocial-wrapper" class="{{ $mostrarChefePsicossocial ? '' : 'hidden' }}">
                <label for="chefe_nucleo_psicossocial" class="inline-flex items-start gap-3">
                    <input id="chefe_nucleo_psicossocial" type="checkbox" value="1" class="mt-1 rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="chefe_nucleo_psicossocial" {{ old('chefe_nucleo_psicossocial', $usuario->hasDirectPermission('acesso irrestrito psicossocial')) ? 'checked' : '' }}>
